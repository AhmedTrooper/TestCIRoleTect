...
document.addEventListener('DOMContentLoaded', () => {
  ...
  modeToggle.addEventListener('change', async () => {
    ...
    updateModeUI(isAdvanced);
    await browser.storage.local.set({ uiMode: isAdvanced ? 'advanced' : 'simple' });
  });
  ...
  async function saveSiteMaps() {
    await browser.storage.local.set({ siteMaps });
    renderSiteMaps();
  }
  ...
  siteSelector.addEventListener('change', async () => {
    await browser.storage.local.set({ activeSiteIndex: siteSelector.value });
  });
  ...
  safeRestoreBtn.addEventListener('click', async () => {
    ...
    const result = await browser.storage.local.get(['siteMaps']);
    ...
    await browser.storage.local.set({ siteMaps: mergedMaps });
  ...
  document.getElementById('interactiveBtn').addEventListener('click', async () => {
    ...
    try {
      const response = await browser.runtime.sendMessage({ action: "START_INTERACTIVE", excludeSelector });
      if (response && response.success) {
        window.close();
      } else {
